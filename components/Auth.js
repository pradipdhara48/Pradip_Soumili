...
export default function Auth() {
  ...
  // --- এখানে handleSignIn ফাংশনটি যুক্ত করা হলো ---
  ...
  if (user) {
    ...
    return (
      <div className="flex h-screen w-full bg-[#f1f5f9] text-[#1e293b] font-sans antialiased overflow-hidden">
        <aside className="w-64 bg-[#1c2434] text-[#dee4ee] flex flex-col justify-between shrink-0 shadow-xl z-20">
          <div>
            <div className="p-6 border-b border-[#2e3a47] flex items-center gap-3">
            ...
            <nav className="p-4 space-y-1">
              ...
              <button onClick={() => handleMenuChange('overview')} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeMenu === 'overview' ? 'bg-[#333a48] text-white shadow-sm' : 'text-gray-400 hover:bg-[#333a48]/50 hover:text-white'}`}>📊 Dashboard Overview</button>
              <button onClick={() => handleMenuChange('couple_hero')} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeMenu === 'couple_hero' ? 'bg-[#333a48] text-white shadow-sm' : 'text-gray-400 hover:bg-[#333a48]/50 hover:text-white'}`}>💑 Couple & Homepage</button>
              <button onClick={() => handleMenuChange('events')} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeMenu === 'events' ? 'bg-[#333a48] text-white shadow-sm' : 'text-gray-400 hover:bg-[#333a48]/50 hover:text-white'}`}>📅 Ceremony & Reception</button>
              <button onClick={() => handleMenuChange('story')} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeMenu === 'story' ? 'bg-[#333a48] text-white shadow-sm' : 'text-gray-400 hover:bg-[#333a48]/50 hover:text-white'}`}>📖 Our Love Story</button>
              <button onClick={() => handleMenuChange('gallery')} className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeMenu === 'gallery' ? 'bg-[#333a48] text-white shadow-sm' : 'text-gray-400 hover:bg-[#333a48]/50 hover:text-white'}`}><div className="flex items-center gap-3">📸 Live Feed & Photos</div><span className="text-xs bg-blue-600 px-2 py-0.5 rounded-full text-white">{posts.length}</span></button>
              <button onClick={() => handleMenuChange('rsvp')} className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeMenu === 'rsvp' ? 'bg-[#333a48] text-white shadow-sm' : 'text-gray-400 hover:bg-[#333a48]/50 hover:text-white'}`}><div className="flex items-center gap-3">💌 RSVP Guests</div><span className="text-xs bg-emerald-600 px-2 py-0.5 rounded-full text-white">{rsvps.length}</span></button>
              <button onClick={() => handleMenuChange('database')} className={`w-full mt-4 flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activeMenu === 'database' ? 'bg-rose-600 text-white shadow-sm' : 'text-rose-400 hover:bg-rose-500/20 hover:text-rose-300'}`}><div className="flex items-center gap-3">🧹 Clean Database</div></button>
            </nav>
          </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-8 shrink-0">
            <h2 className="text-lg font-bold text-gray-800 capitalize">{activeMenu.replace('_', ' ')}</h2>
            <a href="/" target="_blank" className="text-xs font-semibold text-blue-600 hover:underline">↗ View Live Website</a>
          </header>
          ...
          <main className="flex-1 overflow-y-auto p-8 bg-[#f1f5f9]">
            ...
            {activeMenu === 'couple_hero' && (
              <form onSubmit={handleSaveConfig} className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm space-y-5 max-w-3xl">
                ...
                <button type="submit" disabled={savingConfig} className="px-6 py-2.5 bg-emerald-600 text-white rounded-lg text-sm font-semibold cursor-pointer shadow mt-4">{savingConfig ? 'Saving...' : '💾 Save & Publish Changes'}</button>
              </form>
            )}

            {activeMenu === 'events' && (
              <form onSubmit={handleSaveConfig} className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm space-y-6 max-w-4xl">
                ...
                <div className="p-4 bg-gray-50 rounded-xl border space-y-3"><h4 className="font-bold text-sm text-blue-600">The Ceremony</h4><div className="grid grid-cols-2 gap-3"><input type="text" placeholder="Title" value={config.ceremony_title || ''} onChange={(e) => setConfig({ ...config, ceremony_title: e.target.value })} className="p-2 border rounded-lg text-xs bg-white" /><input type="text" placeholder="Time" value={config.ceremony_time || ''} onChange={(e) => setConfig({ ...config, ceremony_time: e.target.value })} className="p-2 border rounded-lg text-xs bg-white" /></div><input type="text" placeholder="Venue Name" value={config.ceremony_venue || ''} onChange={(e) => setConfig({ ...config, ceremony_venue: e.target.value })} className="w-full p-2 border rounded-lg text-xs bg-white" /><input type="text" placeholder="Address" value={config.ceremony_address || ''} onChange={(e) => setConfig({ ...config, ceremony_address: e.target.value })} className="w-full p-2 border rounded-lg text-xs bg-white" /></div>
                <div className="p-4 bg-gray-50 rounded-xl border space-y-3"><h4 className="font-bold text-sm text-indigo-600">The Reception</h4><div className="grid grid-cols-2 gap-3"><input type="text" placeholder="Title" value={config.reception_title || ''} onChange={(e) => setConfig({ ...config, reception_title: e.target.value })} className="p-2 border rounded-lg text-xs bg-white" /><input type="text" placeholder="Time" value={config.reception_time || ''} onChange={(e) => setConfig({ ...config, reception_time: e.target.value })} className="p-2 border rounded-lg text-xs bg-white" /></div><input type="text" placeholder="Venue Name" value={config.reception_venue || ''} onChange={(e) => setConfig({ ...config, reception_venue: e.target.value })} className="w-full p-2 border rounded-lg text-xs bg-white" /><input type="text" placeholder="Address" value={config.reception_address || ''} onChange={(e) => setConfig({ ...config, reception_address: e.target.value })} className="w-full p-2 border rounded-lg text-xs bg-white" /></div>
                ...
                <button type="submit" disabled={savingConfig} className="px-6 py-2.5 bg-emerald-600 text-white rounded-lg text-sm font-semibold cursor-pointer shadow mt-4">{savingConfig ? 'Saving...' : '💾 Save Event Details'}</button>
              </form>
            )}
              ...
              <form onSubmit={handleSaveConfig} className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm space-y-5 max-w-3xl">
                ...
                <div className="space-y-4">
                  {(config.story || []).map((item, idx) => (
                    <div key={idx} className="p-4 border rounded-xl bg-gray-50 relative space-y-3"><button type="button" onClick={() => removeStoryTimeline(idx)} className="absolute top-3 right-3 text-xs text-rose-500 font-bold hover:underline cursor-pointer">Remove ✕</button><div className="grid grid-cols-3 gap-3 pr-16"><input type="text" placeholder="Year" value={item.year || ''} onChange={(e) => handleStoryChange(idx, 'year', e.target.value)} className="p-2 border rounded-lg text-xs bg-white font-bold" /><input type="text" placeholder="Title" value={item.title || ''} onChange={(e) => handleStoryChange(idx, 'title', e.target.value)} className="col-span-2 p-2 border rounded-lg text-xs bg-white font-bold" /></div><textarea rows={2} placeholder="Story Details..." value={item.text || ''} onChange={(e) => handleStoryChange(idx, 'text', e.target.value)} className="w-full p-2 border rounded-lg text-xs bg-white" /></div>
                  ))}
                </div>
                <button type="submit" disabled={savingConfig} className="px-6 py-2.5 bg-emerald-600 text-white rounded-lg text-sm font-semibold cursor-pointer shadow">{savingConfig ? 'Saving...' : '💾 Save Story Timeline'}</button>
              </form>
            )}

            {activeMenu === 'gallery' && (
              <div className="space-y-6 max-w-4xl">
                <form onSubmit={handleUpload} className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm space-y-4"><h3 className="text-base font-bold text-gray-800">Upload to Live Feed</h3><input type="file" accept="image/*" onChange={(e) => setFile(e.target.files[0])} required className="text-sm" /><textarea placeholder="Write a sweet caption..." value={caption} onChange={(e) => setCaption(e.target.value)} rows={2} className="w-full p-2.5 border rounded-lg text-sm bg-gray-50" /><button type="submit" disabled={uploading} className="px-5 py-2.5 bg-blue-600 text-white rounded-lg text-xs font-semibold shadow cursor-pointer">{uploading ? 'Uploading...' : 'Publish Photo 🚀'}</button></form>
            ...
            {activeMenu === 'rsvp' && (
              <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm max-w-4xl space-y-4">
                <h3 className="text-base font-bold text-gray-800 border-b pb-3">Guest RSVPs</h3>
                <div className="space-y-3">
                  {rsvps.map((rsvp) => (
                    <div key={rsvp.id} className="p-4 border rounded-xl bg-gray-50 flex items-start justify-between"><div><h4 className="text-sm font-bold text-gray-900">{rsvp.name} ({rsvp.guests_count || 1} Guests)</h4><p className="text-xs text-gray-500 mt-0.5">{rsvp.email || 'No email'}</p><p className="text-xs text-gray-700 mt-2 bg-white p-2 rounded border">{rsvp.message || 'No message'}</p></div><span className={`text-xs px-2.5 py-1 rounded-full font-bold ${rsvp.attending ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>{rsvp.attending ? 'Attending' : 'Not Attending'}</span></div>
                  ))}
